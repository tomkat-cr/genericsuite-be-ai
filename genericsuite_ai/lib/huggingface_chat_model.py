...
def _is_huggingface_endpoint(llm: Any) -> bool:
    ...
    return isinstance(llm, GsHuggingFaceEndpoint)
...
class GsChatHuggingFace(BaseChatModel):
    ...
    def validate_llm(self) -> Self:
        if (
            ...
            raise TypeError(
                ...
                "GsHuggingFaceEndpoint, HuggingFaceHub, HuggingFacePipeline "
        ...
        **kwargs: Any,
        ...
        if _is_huggingface_textgen_inference(self.llm):
            ...
            llm_result = self.llm._generate(
                prompts=[llm_input], stop=stop, run_manager=run_manager,
                **kwargs
            )
    ...
    def _to_chat_prompt(
        ...
        """
        Convert a list of messages into a prompt format expected by wrappedLLM.
        """
        if not messages:
    ...
    def _resolve_model_id(self) -> None:
        ...
        if not self.model_id:
            raise ValueError(
                ...
                "Make sure that your Hugging Face token has access to the"
                " endpoint."
            )
        ...
        **kwargs: Any,
        ...
        if tool_choice is not None and tool_choice:
            if len(formatted_tools) != 1:
                raise ValueError(
                    "When specifying `tool_choice`, you must provide"
                    " exactly one "
            ...
            if isinstance(tool_choice, str):
                ...
                if (
                    ...
                    raise ValueError(
                        f"Tool choice {tool_choice} was specified,"
                        " but the only provided tool"
                        f" was {formatted_tools[0]['function']['name']}."
                    )
            else:
                raise ValueError(
                    f"Unrecognized tool_choice type. Expected str, bool"
                    f" or dict. Received: {tool_choice}"
                )
